...
def POS(xp, x):
    ...
    # Ensure t[0] and t[1] are scalars
    t0 = t[0][0]
    t1 = t[1][0]

    return np.array([t0, t1]), s
...
def align_img(img, lm, lm3D, mask=None, target_size=224., rescale_factor=102.):
    ...
    # Debug statements to check the types and values
    print(f"w0: {w0}, type: {type(w0)}")
    print(f"h0: {h0}, type: {type(h0)}")
    print(f"s: {s}, type: {type(s)}")
    print(f"t: {t}, type: {type(t)}")
    print(f"t[0]: {t[0]}, type: {type(t[0])}")
    print(f"t[1]: {t[1]}, type: {type(t[1])}")

    # Ensure t[0] and t[1] are scalars
    if not isinstance(t[0], (int, float)) or not isinstance(t[1], (int, float)):
        raise ValueError("t[0] and t[1] must be scalar values")
